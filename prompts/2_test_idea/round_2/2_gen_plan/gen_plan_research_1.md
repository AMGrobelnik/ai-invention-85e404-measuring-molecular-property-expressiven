...
type: research
objective: >-
  Definitively clarify the theoretical distinction between r-rounds-of-1-WL and k-dimensional WL (Maron et al. 2019), verify
  whether r-rounds-of-1-WL converges for drug-like molecular graphs, and identify prior work on per-property WL collision/variance
  metrics.
approach: >-
  Conduct targeted literature search on: (1) theoretical proofs or counterexamples showing whether standard 1-WL run to convergence
  is equivalent to running 1-WL for k rounds on molecular graphs; (2) papers measuring per-property WL collision rates on
  real molecular datasets (particularly Zhu et al. 2022 'Rethinking the Expressive Power of GNNs for Molecular Graphs' and
  BREC benchmark variants); (3) clarification of which GNN architectures implement k-dimensional WL (Maron's k-GNN, NGNN,
  I²-GNN) vs. which implement iterated 1-WL message passing. Answer these specific questions: (a) For small molecular graphs
  (drug-like compounds in QM9), does running 1-WL iterations r=1,2,3 each reach a different fixed partition, or does the partition
  stabilize at r=1 and adding rounds provides no new refinement? (b) Are the measured r-round-of-1-WL variance floors bounds
  on 1-WL-message-passing GNNs or on k-WL-bounded GNNs? (c) What does prior literature report about per-property collision
  rates and variance explanations for molecular benchmarks, and how does this work differ? Deliverable: research_out.json
  with clear answers to (a)–(c), correctly positioned contribution vs. prior work, and explicit framing of what architectural
  classes the r-round-of-1-WL floors actually bound.
depends_on: []
</artifact_direction>
